...
def match_sublist(the_list, to_match):
    ...
    if len(matched_list) == 0:
        ...
        for index in range(len(the_list)):
            if the_list[index].startswith(last) and to_match[:-1] == the_list[index-len_to_match+1:index]:
                matched_list += [(index, index + len_to_match - 1)]
                fix_bug.append([(to_match, the_list[index-len_to_match+1:index+1]),(index, index + len_to_match - 1)])
    
    return matched_list, fix_bug
...
class EntityRecord(Record):
    ...
    def to_string(self, instance, token_list):
        entity_list = list()
        for i, pred_record in enumerate(instance):
            record_type, record_text = pred_record['type'], pred_record['text']

            if record_text == "":
                logger.info(f"Empty string Extraction - line {i}: ({record_type}:{record_text}). {token_list}")
                continue

            _, fix_bug = match_sublist(token_list, self.span_to_token(record_text))
            if len(fix_bug) != 0:
                logger.info(f"Fix bug - line {i}: ({fix_bug[0][0][0]} -> {fix_bug[0][0][1]}). {token_list}")
                record_text = ' '.join(fix_bug[0][0][1])
            entity_list += [(record_type, record_text)]

        return entity_list


    def to_offset(self, instance, token_list):
        map_strategy_dict = {
    ...
    def record_to_offset_first_role(self, instance, token_list):
        ...
        for i, pred_record in enumerate(instance):
            record_type, record_text = pred_record['type'], pred_record['text']
            if record_text == "":
                logger.info(f"Empty offset Extraction - line {i}: ({record_type}:{record_text}). {token_list}")
                continue

            matched_list, fix_bug = match_sublist(token_list, self.span_to_token(record_text))
            if len(fix_bug) != 0:
                logger.info(f"Fix bug - line {i}: ({fix_bug}). {token_list}")

            if len(matched_list) == 0:
                logger.info(f"[Cannot reconstruct] - line {i}: {pred_record}. {token_list}")
                continue
...
class RelationRecord(Record):
    ...
    def to_string(self, instance, token_list):
        ...
        for i, record in enumerate(instance):
            ...
            for role_type, text_str in record['roles'][:2]:
                if text_str == "":
                    logger.info(f"Empty string Extraction - line {i}: ({relation_type}-{role_type}:{text_str}). {token_list}")
                else:
                    _, fix_bug = match_sublist(token_list, self.span_to_token(text_str))
                    if len(fix_bug) != 0:
                        logger.info(f"Fix bug - line {i}: ({fix_bug[0][0][0]} -> {fix_bug[0][0][1]}). {token_list}")
                        text_str = ' '.join(fix_bug[0][0][1])
    ...
    def record_to_offset_first_role(self, instance, token_list):
        ...
        for i, record in enumerate(instance):
            ...
            relation = [relation_type]
            for role_type, text_str in record['roles'][:2]:
                matched_list, fix_bug = match_sublist(token_list, self.span_to_token(text_str))
                if text_str == "":
                    logger.info(f"Empty offset Extraction - line {i}: ({relation_type}-{role_type}:{text_str}). {token_list}")
                    break
                else:
                    if len(fix_bug) != 0:
                        logger.info(f"Fix bug - line {i}: ({fix_bug}). {token_list}")
    ...
    def record_to_offset_closest_role(self, instance, token_list):
        ...
        for i, record in enumerate(instance):
            ...
            arg1_matched_list, arg1_fix_bug = match_sublist(token_list, self.span_to_token(arg1_text))
            arg2_matched_list, arg2_fix_bug = match_sublist(token_list, self.span_to_token(arg2_text))

            if arg1_text == "":
                logger.info(f"Empty offset Extraction - line {i}: ({relation_type}-{arg1_type}:{arg1_text}). {token_list}")
                break
            if arg2_text == "":
                logger.info(f"Empty offset Extraction - line {i}: ({relation_type}-{arg2_type}:{arg2_text}). {token_list}")
                break

            if len(arg1_fix_bug) != 0:
                logger.info(f"Fix bug - line {i}: ({arg1_fix_bug}). {token_list}")
            if len(arg1_matched_list) == 0:
                logger.info(f"[Cannot reconstruct] - line {i}: ({relation_type}-{arg1_type}:{arg1_text}). {token_list}")
                break
            if len(arg2_fix_bug) != 0:
                logger.info(f"Fix bug - line {i}: ({arg2_fix_bug}). {token_list}")
...
class EventRecord(Record):
    ...
    def to_string(self, instance, token_list):
        ...
        event_list = list()
        for i, record in enumerate(instance):
            trigger_type, trigger_text = record['type'], record['trigger']

            if trigger_text == "":
                logger.info(f"Empty string Extraction - line {i}: ({trigger_type}:{trigger_text}). {token_list}")
                continue

            _, fix_bug = match_sublist(token_list, self.span_to_token(trigger_text))
            if len(fix_bug) != 0:
                logger.info(f"Fix bug - line {i}: ({fix_bug[0][0][0]} -> {fix_bug[0][0][1]}). {token_list}")
                trigger_text = ' '.join(fix_bug[0][0][1])

            event = {'type': trigger_type, 'trigger': trigger_text, 'roles': list()}

            for role_type, text_str in record['roles']:
                if text_str == "":
                    logger.info(f"Empty string Extraction - line {i}: ({trigger_type}-{role_type}:{text_str}). {token_list}")
                    continue
                ...
                event['roles'].append((role_type, text_str))
            event_list += [event]
    ...
    def record_to_offset_first_role(self, instance, token_list):
        ...
        for i, record in enumerate(instance):
            ...
            if trigger == "":
                logger.info(f"Empty offset Extraction - line {i}: ({event_type}:{trigger}). {token_list}")
                continue
            matched_list, fix_bug = match_sublist(token_list, self.span_to_token(trigger))
            ...
            for role_type, text_str in record['roles']:
                matched_list1, fix_bug1 = match_sublist(token_list, self.span_to_token(text_str))
                if text_str == "":
                    logger.info(f"Empty offset Extraction - line {i}: ({trigger}-{role_type}:{text_str}). {token_list}")
                    continue
                if len(fix_bug1) != 0:
                    logger.info(f"Fix bug - line {i}: ({fix_bug1}). {token_list}")
                if len(matched_list1) == 0:
                    logger.info(f"[Cannot reconstruct] - line {i}: ({event_type}-{role_type}:{text_str}). {token_list}")
                    continue
                pred_record['roles'] += [(role_type, get_index_tuple(matched_list1[0]))]
