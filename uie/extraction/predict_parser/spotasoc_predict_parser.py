...
class SpotAsocPredictParser(PredictParser):
    '''
    def change_spotasoc2extraid(self, raw_list):
        spot_num = 0
        asoc_num = 0
        res_list =[]
        
        for r in raw_list:
            r_list = self.tokenizer.tokenize(r)
            for i, token in enumerate(r_list):
                if token == spot_prompt:
                    spot_num += 1
                    if spot_num % 2 == 1:
                        r_list[i] = type_start
                    else:
                        r_list[i] = type_end
                elif token == asoc_prompt:
                    asoc_num += 1
                    if asoc_num % 2 == 1:
                        r_list[i] = type_start
                    else:
                        r_list[i] = type_end
            res_list.append(self.tokenizer.decode(self.tokenizer.encode(r_list, add_special_tokens=False, is_split_into_words=True)))
        return res_list
    '''
